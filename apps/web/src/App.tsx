      ...
      <div style={{ position: 'absolute', inset: 0, background: C.bg, color: C.text }}>
        ...
        <div style={{ position: 'absolute', top: 42, left: 0, right: 0, bottom: 0 }}>
          <BootScreen
             ...
             *
             * 用户 2026-09-21："启动动画为 5-15 秒范围内，当前太慢了"。
             * 宿主的 `boot.run{pacingMs}` 是"**每个模块**处理完后 sleep 这么多"，
             * 而模块是 5 个（地图引擎/通信链路/AI引擎/集群管理/数据服务）→ 5 × 1500 ≈ **8 秒**，
             * 实测口径与上一轮一致（探针本身约 0.7 秒，合计约 8.2 秒），落在 5~15 秒里。
             * 上一轮的值是 6000（≈30 秒，2026-09-18 用户要"30 秒左右"撑满）；再上一轮 400（≈2 秒）。
             * 注意：`demo.ps1 runAll` / `flow.runAll` 走的是它们自己的 pacing（默认 0），**不受这里影响**
             * —— 一键演示不会因此变慢。
             */
            onStart={() => void run('boot.run', { pacingMs: 1500 })}
